...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        self.randomCoinNum = 3 # random.randint(20, 1000)
    ...
    def on_draw(self):
        ...
        arcade.draw_line(0, SCREEN_HEIGHT, SCREEN_WIDTH - (SCREEN_WIDTH * self.time/20), SCREEN_HEIGHT, arcade.color.LIME, 50)

        # arcade.draw_text(f"Time Enlapsed: {self.time}", 10, 670, arcade.color.BLACK, 14)
        self.coin_list.update()

        arcade.draw_text(self.YouWin, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, arcade.color.BLACK, 30,
                         anchor_x="center")

        arcade.draw_text(self.RestartGame, SCREEN_WIDTH / 2, SCREEN_HEIGHT/2 - 70, arcade.color.BLACK, 20,
                         anchor_x="center")

        arcade.draw_text(self.StopGame, SCREEN_WIDTH / 2, SCREEN_HEIGHT/2 - 120, arcade.color.BLACK, 20,
                         anchor_x="center")
    ...
    def on_update(self, delta_time):
        ...
        if self.score == self.randomCoinNum:
            self.gameOver = True
            self.welcomeText = ""
            self.YouWin = "Game over!"
            self.StopGame = "Press Q to quit"
            self.RestartGame = "Press R to restart"
    ...
    def on_key_press(self, key, modifiers):
        ...
        if self.gameOver == True and key == arcade.key.Q:
            arcade.close_window()
